...
export default function AdminDashboard(): React.JSX.Element {
  ...
  return (
    // 💡 画面全体のベース背景を「あいす・あいぼりー」にし、縦いっぱいに広げる
    <div className="min-h-screen bg-[#f3deb9] text-gray-800 flex flex-col font-yuji">
      
      {/* ─── 上部ヘッダー（ベースカラー：白い背景にういろう・ピンクの文字と境界線） ─── */}
      <header className="bg-white text-[#e8aaa3] px-6 py-4 flex justify-between items-center shadow-sm border-b border-[#e8aaa3] z-10">
        ...
        <div className="flex items-center space-x-6 text-sm">
          ...
          <button className="border border-gray-400 hover:bg-white/40 px-3 py-1 rounded transition-colors text-xs text-gray-600">
            ログアウト
          </button>
        </div>
      </header>

      {/* ─── メインコンテンツ（2カラム構成・flex-1 で下まで引き伸ばす） ─── */}
      <div className="flex-1 flex flex-col md:flex-row">
        ...
        <aside className="w-full md:w-80 bg-[#aeac78] text-[#f3deb9] p-6 flex flex-col space-y-6 shrink-0">
          <div>
            <h2 className="text-2xl font-bold tracking-wide border-b border-white/20 pb-2 text-[#f3deb9]">
              管理画面
            </h2>
          </div>
          ...
          <nav className="flex flex-col space-y-2">
            ...
            <button
              ...
              className={`w-full text-left px-4 py-3 rounded-lg font-bold transition-all flex justify-between items-center ${
                activeTab === 'contacts' ? 'bg-[#f3deb9] text-[#aeac78] shadow-sm' : 'hover:bg-white/10 text-[#f3deb9]'
              }`}
            >
              <span>お問い合わせ管理</span>
            ...
            <button
              ...
              className={`w-full text-left px-4 py-3 rounded-lg font-bold transition-all flex justify-between items-center ${
                activeTab === 'news' ? 'bg-[#f3deb9] text-[#aeac78] shadow-sm' : 'hover:bg-white/10 text-[#f3deb9]'
              }`}
            >
              <span>お知らせ管理</span>
          ...
          <div className="pt-4 border-t border-white/10">
            <button className="w-full bg-white hover:bg-[#f3deb9] text-[#e8aaa3] py-2.5 rounded-lg font-bold text-sm transition-colors shadow-sm tracking-wider">
              ＋ 新しいお知らせ追加する
            </button>
          </div>
        </aside>

        {/* 右側：タイムラインエリア（ベースカラー：あいす・あいぼりーを全面に満たす） */}
        <main className="flex-1 p-6 md:p-8 bg-[#f3deb9]">
          <div className="max-w-4xl mx-auto">
            <div className="flex justify-between items-center mb-6 border-b border-gray-300/60 pb-3">
              <h3 className="text-xl font-bold text-gray-700">
                {activeTab === 'all' && '総合タイムライン（最新順）'}
                {activeTab === 'contacts' && '届いたお問い合わせ（最新順）'}
                {activeTab === 'news' && 'お知らせ一覧'}
              </h3>
              <span className="text-xs text-gray-500">自動更新有効</span>
            </div>
              ...
              {filteredTimeline.length === 0 ? (
                ...
                filteredTimeline.map((item) => (
                  <div 
                    ...
                    className={`p-5 rounded-lg bg-white border border-gray-200/80 transition-all duration-200 ${
                      item.type === 'contact' 
                        ? 'border-l-4 border-l-[#e8aaa3] hover:shadow-sm' 
                        : 'border-l-4 border-l-[#aeac78] hover:shadow-sm' 
                    }`}
                  >
                    {/* アイテムヘッダー */}
